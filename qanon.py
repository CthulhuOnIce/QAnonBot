...
def rss2markdown(rss):
	...
	rss = rss.replace("</strong>", "")
	rss = rss.replace("&#09;", "  ")
	rss = re.sub(LINKREPLACEPATTERN, r"\2", rss)
	return rss
...
def get_new_drops():
	# update cache and return new drops
	global LASTDROP
	global DROPCACHE
	new_drops = []
	try:
		feed = feedparser.parse(STANDARDFEED).entries
		if feed[0]["title"] == LASTDROP.rss["title"]:	return []
		for entry in feed:
...
def QDropListToText(drops):
	textlist = []
	for drop in drops:
		textlist.append(drop.body)
	return textlist